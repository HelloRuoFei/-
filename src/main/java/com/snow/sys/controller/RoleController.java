...
public class RoleController {
	...
	public ResultObj addRole(RoleVo roleVo) {
		try {
			...
			roleVo.setCreatetime(new Date());//获取时间
			
			//这里可以传notivcevo对象也可以传role对象
			this.roleService.save(roleVo);//存储
	...
	public ResultObj updateRole(RoleVo roleVo) {
		try {
			this.roleService.updateById(roleVo);//更新。只更新标题和内容其他不变
	...
	public DataGridView initPermissionByRoleId(Integer roleId) {
		...
		List<Permission> allPermissions = permissionService.list(queryWrapper);//权限和菜单所有可用的
		/**
